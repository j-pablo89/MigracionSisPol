...
controller.login = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    conn.query(
      "SELECT id_usuario, Usuario, Contrasenia, Email, Telefono, Nombre, Apellido, Dni, Avatar_url, Fecha_baja, permiso_principal AS Permiso, nombre_rol FROM pol_usuarios INNER JOIN pol_persona USING(id_Persona) INNER JOIN pol_usuarioperfiles USING(id_usuario) INNER JOIN pol_roles USING(id_rol) WHERE Usuario = ? AND pol_usuarios.Estado = 1",
      [username],
      (err, users) => {
        ...
        if (user.Fecha_baja) {
          ...
          if (fechaBaja <= hoy) {
            ...
            conn.query(
              ...
              [user.id_usuario, EstadoConexion],
              (err) => {
                if (err) {
                  console.error("Error al registrar cuenta vencida:", err);
                }
                return res.redirect(
                  "/error401?error_msg=Su%20cuenta%20ha%20vencido.%20Contacte%20a%20un%20administrador%20para%20restablecerla.",
                );
              },
            );
            return;
        ...
        const passwordIgualUsuario = bcrypt.compareSync(
          user.Usuario + PEPPER,
          user.Contrasenia,
        );
        if (!passwordIsValid) {
          ...
          conn.query(
            ...
            (err) => {
              ...
              return res.redirect(
                "/error401?error_msg=Credenciales%20inválidas",
              );
            },
          );
        ...
        if (req.session.nombre_rol !== "ADMINISTRADOR") {
          conn.query(
            "SELECT uu.id_Unidades, un.Detalle AS Detalle FROM pol_usuariounidades uu INNER JOIN pol_unidades un ON uu.id_unidades = un.id_Unidades WHERE id_usuario = ? AND Estado = 1",
            [user.id_usuario],
            (err, unidades) => {
              if (err) {
...
controller.inicio = (req, res) => {
  req.getConnection((err, conn) => {
    ...
    conn.query("SELECT MONTH(Fecha_alta) AS mes, COUNT(*) AS cantidad FROM pol_internolegajo WHERE YEAR(Fecha_alta) = YEAR(CURDATE()) GROUP BY MONTH(Fecha_alta) ORDER BY mes", (err, resultadoIngresos) => {
        if (err) {
          return res.json(err);
        }
        ...
        conn.query(
          "SELECT MONTH(Fecha_movimiento) AS mes, COUNT(*) AS cantidad FROM pol_internomovimiento WHERE YEAR(Fecha_movimiento) = YEAR(CURDATE()) AND Tipo_movimiento = 'EGRESO' OR (Tipo_movimiento = 'MOVIMIENTO' AND Unidad_Destino = '50') GROUP BY MONTH(Fecha_movimiento) ORDER BY mes",
                  ...
                  (err, ingresosAnioActual) => {
                    ...
                    conn.query(
                      ...
                      (err, ingresosTotales) => {
                        ...
                        conn.query(
                          "SELECT CASE WHEN u.region IS NULL OR u.region = '' THEN 'OTROS' ELSE u.region END AS region, COUNT(*) AS cantidad " +
                          "FROM pol_internolegajo il " +
                          "INNER JOIN (SELECT * FROM pol_internomovimiento m WHERE m.id_InternoMovimiento IN (SELECT MAX(id_InternoMovimiento) FROM pol_internomovimiento GROUP BY id_InternoLegajo)) im ON im.id_InternoLegajo = il.id_InternoLegajo " +
                          "INNER JOIN pol_unidades u ON u.id_Unidades = im.Unidad_Destino " +
                          "WHERE il.Estado = 1 AND im.Unidad_Destino > 99 " +
                          "GROUP BY CASE WHEN u.region IS NULL OR u.region = '' THEN 'OTROS' ELSE u.region END " +
                          "ORDER BY region",
                          (err, detenidosPorRegion) => {
                            if (err) {
                              return res.json(err);
                            }

                            const labelsRegiones = detenidosPorRegion.map(
                              (r) => r.region,
                            );
                            const dataRegiones = detenidosPorRegion.map(
                            ...
                            return res.render("inicio", {
                              ...
                              ingresosAnioActual,
                              ingresosTotales,
                              labelsRegiones: JSON.stringify(labelsRegiones),
                              dataRegiones: JSON.stringify(dataRegiones),
                            });
                          },
                        );
                      },
...
controller.detenidosPorRegion = (req, res) => {
  req.getConnection((err, conn) => {
    if (err) return res.json(err);

    const region = req.params.region;
    const condicionRegion =  region === "OTROS" ? "(u.region IS NULL OR u.region = '')" : "u.region = ?";
    const params = region === "OTROS" ? [] : [region];

    conn.query(
      "SELECT il.id_InternoLegajo, pp.Apellido, pp.Nombre, pp.Dni, u.Detalle AS comisaria " +
      "FROM pol_persona pp " +
      "INNER JOIN pol_internolegajo il USING(id_persona) " +
      "INNER JOIN (SELECT * FROM pol_internomovimiento m WHERE m.id_InternoMovimiento IN (SELECT MAX(id_InternoMovimiento) FROM pol_internomovimiento GROUP BY id_InternoLegajo)) im ON im.id_InternoLegajo = il.id_InternoLegajo " +
      "INNER JOIN pol_unidades u ON u.id_Unidades = im.Unidad_Destino " +
      "WHERE il.Estado = 1 AND im.Unidad_Destino != 50 AND " + condicionRegion + " " +
      "ORDER BY u.Detalle, pp.Apellido, pp.Nombre",
      params,
      (err, detenidos) => {
        if (err) {
          return res.json(err);
        }
        return res.json({ region, detenidos });
      },
    );
  });
};
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(sqlIngresos, [anio], (err, resultadoIngresos) => {
      if (err)
        return res
          .status(500)
          .json({ error: "Error en la consulta de ingresos" });

      conn.query(sqlEgresos, [anio], (err, resultadoEgresos) => {
        if (err)
          return res
            .status(500)
            .json({ error: "Error en la consulta de egresos" });
...
controller.listarUsuarios = (req, res) => {
  req.getConnection((err, conn) => {
    conn.query(
      `SELECT id_usuario, pol_persona.id_Persona, Usuario, Email, Apellido, Nombre, Dni, Avatar_url, COALESCE(permiso_principal, 'SIN PERMISOS') AS Permiso, COALESCE(nombre_rol, 'SIN PERFIL') AS nombre_rol, pol_usuarios.Estado, pol_usuarios.Ubicacion, pol_usuarios.Funcion FROM pol_persona INNER JOIN pol_usuarios USING(id_persona) LEFT JOIN pol_usuarioperfiles USING(id_usuario) LEFT JOIN pol_roles USING(id_rol) ORDER BY pol_usuarios.Estado DESC, CASE WHEN nombre_rol = 'SUPERADMIN' THEN 1 WHEN nombre_rol = 'ADMINISTRADOR' THEN 2 WHEN nombre_rol = 'USUARIO GENERAL' THEN 3 WHEN nombre_rol = 'USUARIO' THEN 4 ELSE 5 END, Apellido, Nombre`,
      (err, usuarios) => {
        if (err) {
          res.json(err);
        }
        conn.query(
          ...
          (err, unidades) => {
            ...
            conn.query(
              "SELECT * FROM pol_usuariounidades INNER JOIN pol_unidades USING(id_Unidades)",
              (err, dependencia) => {
                if (err) {
                  res.json(err);
                }
    ...
    bcrypt.hash(Dni + PEPPER, 10, (err, hashedPassword) => {
      ...
      conn.query(
        ...
        (err, rows) => {
          ...
          conn.query(`UPDATE pol_usuarios SET Contrasenia = ?, debeCambiarClave = 1 WHERE id_usuario = ?`,
  ...
  req.getConnection(async (err, conn) => {
    ...
    conn.query("INSERT INTO pol_persona SET ?", usuarioPersonal,(err, resultado) => {
        if (err) {
          ...
          return resultado
            .status(500)
            .send("Error al guardar datos personales");
        }
        ...
        conn.query(
          ...
          async (err, resultado) => {
            if (err) {
              ...
              return resultado
                .status(500)
                .send("Error al guardar nuevo usuario");
            }
            ...
            const historialData = {
              id_usuario,
              accion: "CREACION",
              datos_nuevos: JSON.stringify(usuarioData),
              usuario_modifica: login,
            };
            conn.query(
              "INSERT INTO pol_usuarios_historial SET ?",
              historialData,
              (err) => {
                if (err) {
            ...
            conn.query(
              "INSERT INTO pol_usuarioperfiles SET ?",
              perfilData,
              (err, resultado) => {
                if (err) {
                  ...
                  return resultado
                    .status(500)
                    .send("Error al guardar nuevo perfil");
                }
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      ...
      (err, rows) => {
        ...
        // Guardar historial
        conn.query(
          `INSERT INTO pol_usuarios_historial (id_usuario, accion, datos_anteriores, datos_nuevos, usuario_modifica) VALUES (?, 'ACTUALIZACION', ?, ?, ?)`,
          [
            id,
          ...
          (err) => {
            ...
            // Actualizar persona
            conn.query(
              "UPDATE pol_persona SET ? WHERE id_persona = ?",
              ...
              (err) => {
                ...
                // Actualizar usuario
                conn.query(
                  `UPDATE pol_usuarios SET Estado = ?, Usuario = ?, Email = ?, Fecha_Baja = ?, Telefono = ?, Ubicacion = ?, Funcion = ? WHERE id_Persona = ?`,
                  [
                    1,
                    Usuario,
                    Email,
...
controller.guardarAcceso = (req, res) => {
  ...
  if (idRol === 3 && nuevas.length === 0) {
    return res
      .status(400)
      .send("Un usuario debe tener al menos una dependencia asignada");
  }
  ...
  req.getConnection((err, conn) => {
    ...
    conn.beginTransaction((err) => {
      ...
      conn.query(
        `SELECT * FROM pol_usuariounidades WHERE id_usuario = ?`,
        [idUsuario],
        (err, rows) => {
          if (err)
          ...
          if (encontrado) {
            conn.query(
              `SELECT id_Unidades FROM pol_usuariounidades WHERE id_usuario = ? AND Estado = 1`,
              [idUsuario],
              (err, actuales) => {
                if (err)
                  return conn.rollback(() =>
                ...
                if (aDesactivar.length > 0) {
                  conn.query(
                    `UPDATE pol_usuariounidades SET Estado = 0, Modifica_autoriza = ? WHERE id_usuario = ? AND id_Unidades IN (?)`,
                    [login, idUsuario, aDesactivar],
                    (err) => {
                      if (err)
                        return conn.rollback(() =>
                ...
                function insertarOReactivar() {
                  ...
                  conn.query(
                    `INSERT INTO pol_usuariounidades (id_usuario, id_Unidades, Estado, Alta_Autoriza, Fecha_alta) VALUES ? ON DUPLICATE KEY UPDATE 
                                    Estado = 1, Modifica_autoriza = VALUES(Alta_Autoriza)`,
                    [valores],
                    (err) => {
                      if (err)
                        return conn.rollback(() =>
                          res
                            .status(500)
                            .send("Error insertando/reactivando unidades"),
                        );
                      actualizarPerfil();
                    },
                  );
            ...
            //Insertar / Reactivar unidades finales
            const valores = nuevas.map((idUnidad) => [
              idUsuario,
              idUnidad,
              1,
            ...
            conn.query(
              `INSERT INTO pol_usuariounidades (id_usuario, id_Unidades, Estado, Alta_Autoriza, Fecha_alta) VALUES ? ON DUPLICATE KEY UPDATE Estado = 1, Modifica_autoriza = VALUES(Alta_Autoriza)`,
              [valores],
              (err, result) => {
                if (err) {
                  console.error("=================================");
      ...
      function actualizarPerfil() {
        conn.query(
          ...
          [idRol, permisoPrincipal, idUsuario],
          (err) => {
            if (err)
              return conn.rollback(() =>
      ...
      function guardarHistorial() {
        ...
        conn.query(
          `INSERT INTO pol_usuarios_historial (id_usuario, accion, datos_nuevos, usuario_modifica) VALUES (?, 'CAMBIO_ACCESO', ?, ?)`,
          [idUsuario, JSON.stringify(datosNuevos), login],
          (err) => {
            if (err)
              return conn.rollback(() =>
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      ...
      (err, rows) => {
        ...
        conn.query(
          "UPDATE pol_usuarios SET Estado = 0 WHERE id_usuario = ?",
          [id],
          (err) => {
            if (err) {
              console.error(err);
            ...
            conn.query(
              "SELECT id_rol FROM pol_roles WHERE Nombre_rol = ? LIMIT 1",
              ["SIN PERFIL"],
              (err, rowsRol) => {
                if (err) {
                  console.error(err);
                  return res
                    .status(500)
                    .send("Error al obtener rol SIN PERFIL");
                }

                const idRolSinPerfil =
                  rowsRol.length > 0 ? rowsRol[0].id_rol : null;
                ...
                conn.query(
                  ...
                  (err) => {
                    if (err) {
                      console.error(
                        "Error al actualizar perfil al desactivar:",
                        err,
                      );
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      ...
      (err, rows) => {
        ...
        conn.query(
          "UPDATE pol_usuarios SET Estado = 1 WHERE id_usuario = ?",
          [id],
          (err) => {
            if (err) {
              console.error(err);
  ...
  req.getConnection(async (err, conn) => {
    ...
    try {
      const [rows] = await conn
        .promise()
        .query("SELECT * FROM pol_usuarios WHERE id_usuario = ?", [id]);
      if (rows.length === 0) {
      ...
      if (nuevaContrasenia === usuario.Usuario) {
        return res
          .status(400)
          .json({ error: "La clave no puede ser igual al usuario" });
      }
      ...
      await conn
        ...
        .query(
          ...
          [
            ...
            JSON.stringify({ Contrasenia: claveHash }),
            login,
          ],
        );
      await conn
        .promise()
        .query(
          "UPDATE pol_usuarios SET Contrasenia = ?, Modifica_autoriza = ?, debeCambiarClave = 0 WHERE id_usuario = ?",
          [claveHash, login, id],
        );
...
controller.guardarDetenido = (req, res) => {
  ...
  let {
    ...
    Oficio_Fecha,
    Causa,
    Fecha_cumple_condena,
    Apellido_victima,
    Nombre_victima,
    Situacion_procesal,
    id_localidad,
    id_provincia,
    id_autoridad_judicial,
    id_comisaria_dependiente,
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      "SELECT * FROM pol_persona pp INNER JOIN pol_internolegajo il USING(id_persona) WHERE pp.Dni = ?",
      [Dni],
      (err, results) => {
        if (results.length > 0) {
        ...
        conn.query(
          ...
          (err, resultado) => {
            ...
            conn.query(
              "INSERT INTO pol_internolegajo SET ?",
              [datosDetenido],
              (err, resultado) => {
                if (err) {
                  ...
                  return resultado
                    .render(500)
                    .send("Error al guardar nuevo detenido");
                }
                ...
                const datosProntuario = {
                  ...
                  Situacion_procesal,
                  Fecha_cumple_condena,
                  id_autoridadjudicial,
                  Alta_autoriza,
                  Estado,
                  Observaciones: Detalle,
                };
                conn.query(
                  "INSERT INTO pol_internoprontuario SET ?",
                  [datosProntuario],
                  (err, resultado) => {
                    if (err) {
                      ...
                      return resultado
                        .render(500)
                        .send("Error al guardar nuevo prontuario");
                    }
                    ...
                    conn.query(
                      "INSERT INTO pol_internoprontuario_historial (id_InternoLegajo, id_InternoProntuario, accion, datos_nuevos, usuario_modifica) VALUES (?, ?, ?, ?, ?)",
                      [
                        id_InternoLegajo,
                        id_InternoProntuario,
                        "CREACION",
                        JSON.stringify(datosProntuario),
                        Alta_autoriza,
                      ],
                      (err) => {
                        if (err) {
                          console.error(
                            "Error al insertar en prontuario historial",
                            err,
                          );
                          return resultado
                            .render(500)
                            .send("Error al guardar en prontuario historial");
                        }
                        conn.query(
                          "INSERT INTO pol_internomovimiento (id_InternoLegajo, id_InternoProntuario, Unidad_Alojado, Unidad_Destino, Tipo_Movimiento, Detalle, Usuario) VALUES (?, ?, ?, ?,'INGRESO',?,?)",
                          [
                            id_InternoLegajo,
                            id_InternoProntuario,
                            Unidad_alojado,
                            Unidad_Destino,
                            JSON.stringify(DetalleMovimiento),
                            usuario,
                          ],
                          (err, result) => {
                            if (err) {
                              console.error(
                                "Error al insertar nuevo movimiento",
                                err,
                              );
                              return resultado
                                .render(500)
                                .send("Error al guardar nuevo movimiento");
                            }
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query("SELECT * FROM pol_localidad", (err, localidades) => {
      ...
      conn.query("SELECT * FROM pol_provincia", (err, provincia) => {
        ...
        conn.query(
          "SELECT pol_internolegajo.id_InternoLegajo, pol_internoLegajo.Personal_Fuerza AS Personal_Fuerza, pol_persona.id_Persona AS IdPersona, pol_persona.Apellido AS Apellido, pol_persona.Nombre AS Nombre, pol_persona.Dni AS Dni, pol_persona.Sexo AS Sexo, pol_persona.Fecha_nacimiento AS Fecha_nacimiento, pol_persona.Domicilio AS Domicilio, localidad.nombre AS Localidad, localidad.id_localidad AS id_localidad, provincia.Nombre AS Provincia, provincia.id_provincia AS id_provincia FROM pol_internoLegajo INNER JOIN pol_persona USING(id_persona) INNER JOIN pol_localidad AS localidad ON localidad.id_localidad = pol_persona.id_localidad INNER JOIN pol_provincia AS provincia ON provincia.id_provincia = localidad.id_provincia WHERE id_internoLegajo = ?",
          [id],
          (err, detenidos) => {
            if (err) {
              return res.json(err);
...
controller.actualizarDetenido = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      "SELECT * FROM pol_persona WHERE id_Persona = ?",
      [id_Persona],
      (err, rows) => {
        if (err) {
        ...
        if (historial.length > 0) {
          conn.query(
            "INSERT INTO pol_internolegajo_historial (id_InternoLegajo,campo,valor_anterior,valor_nuevo,usuario) VALUES ?",
            [
              historial.map((h) => [
                h.id_InternoLegajo,
                h.campo,
            ...
            (err, detenidos) => {
              ...
              conn.query(
                ...
                (err, datos) => {
                  ...
                  // Calcular tiempo restante para edición por cada causa
                  const perfil = req.session && req.session.nombre_rol;
                  const isAdminOrSuperadmin =
                    perfil === "ADMINISTRADOR" || perfil === "SUPERADMIN";
                  ...
                  detenidos = detenidos.map((item) => {
                    ...
                    if (item.Estado === 2 && isAdminOrSuperadmin) {
                      ...
                      canEdit =
                        !Number.isNaN(fechaAltaMs) &&
                        Date.now() - fechaAltaMs <= 120000;
                    }
...
controller.guardarCausaDetenido = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    const datosProntuario = {
      ...
      Causa,
      Victima,
      Unidad_dependencia: id_comisaria_dependiente,
      Fecha_Detencion,
      Fecha_cumple_condena,
      Fecha_hecho,
      Situacion_procesal,
      id_AutoridadJudicial: id_autoridad_judicial,
      Alta_autoriza: login,
      Estado,
      Observaciones: Detalle,
    };
...
controller.actualizarCausaDetenido = (req, res) => {
  ...
  let {
    ...
    Fecha_Detencion,
    Fecha_cumple_condena,
    Fecha_cumple_condena_disabled1,
    Fecha_Hecho,
    Victima,
    Detalle,
  } = req.body;
  req.getConnection((err, conn) => {
    ...
    conn.query(
      "SELECT * FROM pol_internoprontuario WHERE id_InternoProntuario = ?",
      [id_internoprontuario],
      (err, rows) => {
        if (err) {
          console.error("Error al obtener datos actuales", err);
        ...
        rows[0].Oficio_Fecha = rows[0].Oficio_Fecha
          ? new Date(rows[0].Oficio_Fecha).toISOString().slice(0, 10)
          : null;
        rows[0].Fecha_cumple_condena = rows[0].Fecha_cumple_condena
          ? new Date(rows[0].Fecha_cumple_condena).toISOString().slice(0, 10)
          : null;
        rows[0].Fecha_Hecho = rows[0].Fecha_Hecho
          ? new Date(rows[0].Fecha_Hecho).toISOString().slice(0, 10)
          : null;
        rows[0].Fecha_Detencion = rows[0].Fecha_Detencion
          ? new Date(rows[0].Fecha_Detencion).toISOString().slice(0, 10)
          : null;

        const actual = rows[0];

        if (!isAdmin && actual) {
          const fechaAltaMs = actual.Fecha_alta
            ? new Date(actual.Fecha_alta).getTime()
            : null;
          const fechaBase =
            fechaAltaMs ||
            (actual.Fecha_modifica
              ? new Date(actual.Fecha_modifica).getTime()
              : null);
          if (fechaBase && Date.now() - fechaBase > 120000) {
            console.warn(
              "Intento de actualización fuera de tiempo:",
              id_internoprontuario,
            );
        ...
        const datosProntuario = {
          ...
          Victima,
          Fecha_Detencion,
          Fecha_cumple_condena: Fecha_cumple_condena_disabled1,
          Fecha_Hecho,
          Unidad_Dependencia: id_comisaria_dependiente,
          Situacion_procesal,
          id_AutoridadJudicial: id_autoridad_judicial,
        ...
        conn.query(
          "INSERT INTO pol_internoprontuario_historial (id_InternoLegajo, id_InternoProntuario, accion, datos_anteriores, datos_nuevos, usuario_modifica) VALUES (?, ?, ?, ?, ?, ?)",
          [
            id_InternoLegajo,
            id_internoprontuario,
            "MODIFICACION",
          ...
          (err) => {
            ...
            conn.query(
              "UPDATE pol_internoprontuario SET ? WHERE id_InternoProntuario = ?",
              [datosProntuario, id_internoprontuario],
              (err, resultado) => {
                if (err) {
...
controller.cerrarCausaDetenido = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    const tipo = "CIERRE";
    conn.query(
      "INSERT INTO pol_internoprontuario_historial (id_InternoLegajo, id_InternoProntuario, accion, usuario_modifica, Motivo) VALUES (?, ?, ?, ?, ?)",
      [id_InternoLegajo, id_InternoProntuario, tipo, login, motivo],
      (err) => {
        if (err) {
          console.error("Error al insertar en el historial", err);
        }
        conn.query(
          "UPDATE pol_internoprontuario SET Estado = ?, Modifica_autoriza = ?, Detalle_Estado = ? WHERE id_InternoProntuario = ?",
          [Estado, login, motivo, id_InternoProntuario],
          (err) => {
            if (err) {
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      ...
      (err) => {
        ...
        conn.query(
          "UPDATE pol_internoprontuario SET Modifica_autoriza = ?, Estado = ?, Detalle_Estado = ? WHERE id_InternoProntuario = ?",
          [login, Estado, motivo, id_InternoProntuario],
          (err) => {
            if (err) {
              console.error("Error al actualizar prontuario", err);
              return res.status(500).send("Error al actualizar prontuario");
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      ...
      (err, detenido) => {
        ...
        const fechaAlta = registro.Fecha_alta
          ? new Date(registro.Fecha_alta).getTime()
          : null;
...
controller.guardarFotos = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      "SELECT frente, izquierdo, derecho, espalda, Fecha_alta FROM pol_internofotos WHERE id_InternoLegajo = ?",
      [id_InternoLegajo],
      (err, rows) => {
        if (err) {
        ...
        const isAdmin =
          req.session?.nombre_rol === "ADMINISTRADOR" ||
          req.session?.nombre_rol === "SUPERADMIN";
        ...
        const hayFotoNueva =
          files?.frente || files?.izquierdo || files?.derecho || files?.espalda;
        if (!hayFotoNueva) {
          return res.render("agregar_foto", {
            ...
            editTimeRemaining: existente.Fecha_alta
              ? Math.max(
                  0,
                  LIMITE -
                    (Date.now() - new Date(existente.Fecha_alta).getTime()),
                )
              : LIMITE,
          });
        }
...
controller.guardarArchivo = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      insertQuery,
      [id_InternoLegajo, archivoPath, archivo1, login],
      (err, result) => {
        if (err) {
          console.error("Error al insertar en DB:", err);
          return res.redirect(`/agregar_archivo/${internoId}?error=1`);
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      ...
      (err, rows) => {
        ...
        conn.query(
          "UPDATE pol_internolegajo SET Estado = ?, Modifica_autoriza = ? WHERE id_InternoLegajo = ?",
          [Estado, login, id],
          (err, result) => {
            if (err) {
            ...
            const updateQuery =
              "INSERT INTO pol_internomovimiento (id_InternoLegajo, id_InternoProntuario, Unidad_Alojado, Tipo_Movimiento, Detalle, Usuario) VALUES (?, ?, ?, 'EGRESO', ?, ?)";
            conn.query(
              updateQuery,
              [
                id,
...
controller.moverDetenido = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      "SELECT Unidad_alojado FROM pol_internomovimiento WHERE id_InternoLegajo = ? ORDER BY fecha_movimiento DESC LIMIT 1",
      [id],
      (err, rows) => {
        if (err) {
          return res.json(err);
        ...
        conn.query(
          "INSERT INTO pol_internomovimiento (id_InternoLegajo, Unidad_Alojado, Unidad_Destino, Tipo_Movimiento, Detalle, Usuario) VALUES (?, ?, ?, 'MOVIMIENTO',?,?)",
          [
            id,
            unidad_alojado,
            nuevoAlojamiento,
...
controller.regresarDetenido = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      "SELECT Unidad_alojado, Detalle FROM pol_internomovimiento WHERE id_InternoLegajo = ? ORDER BY fecha_movimiento DESC LIMIT 1",
      [id],
      (err, rows) => {
        if (err) {
          return res.json(err);
        ...
        const detalleMovimiento = {
          ...(detalleBase.Detalle &&
          typeof detalleBase.Detalle === "object" &&
          !Array.isArray(detalleBase.Detalle)
            ? detalleBase.Detalle
            : detalleBase),
          observaciones,
        };
        ...
        conn.query(
          "INSERT INTO pol_internomovimiento (id_InternoLegajo, Unidad_Alojado, Unidad_Destino, Tipo_Movimiento, Detalle, Usuario) VALUES (?, ?, ?, 'REGRESA A COMISARIA',?,?)",
          [
            id,
            unidadAlojado,
            unidadAlojado,
...
controller.trasladarDetenido = (req, res) => {
  const id = req.params.id;
  const login = req.session.username;
  let { Movil, personalCustodio, observaciones, id_comisaria_destino } =
    req.body;
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      `SELECT im.id_InternoMovimiento, il.id_Persona, im.id_InternoProntuario, im.Unidad_Alojado, im.Unidad_Destino FROM pol_internomovimiento im INNER JOIN pol_internolegajo il USING(id_InternoLegajo) WHERE il.id_InternoLegajo = ? ORDER BY id_InternoMovimiento DESC LIMIT 1`,
      [id],
      (err, rows) => {
        if (err || !rows.length) {
          console.error(err);
        ...
        conn.query(
          `SELECT p.Apellido, p.Nombre, p.Dni, u1.Detalle AS unidad_origen, u2.Detalle AS unidad_destino FROM pol_internolegajo il INNER JOIN pol_persona p USING(id_Persona) LEFT JOIN pol_unidades u1 ON u1.id_Unidades = ? LEFT JOIN pol_unidades u2 ON u2.id_Unidades = ? WHERE il.id_InternoLegajo = ?`,
          [movimiento.Unidad_Alojado, id_comisaria_destino, id],
          (err, data) => {
            if (err || !data.length) {
              return res.redirect(`/detenidos?error=1`);
            }
            ...
            conn.query(
              "INSERT INTO pol_notificaciones SET ?",
              notificacion,
              (err) => {
                if (err) {
...
controller.obtenerNotificaciones = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    ...
    conn.query(
      `SELECT n.*, IF(nu.leida IS NULL, 0, nu.leida) AS leida FROM pol_notificaciones n INNER JOIN pol_usuariounidades uu ON uu.id_Unidades = n.Unidad_Destino_Codigo AND uu.id_usuario = ? AND uu.Estado = 1 LEFT JOIN pol_notificaciones_usuarios nu ON n.id_Notificacion = nu.id_notificacion AND nu.id_usuario = ? WHERE n.Estado = 'pendiente' ORDER BY n.Fecha_Creacion DESC`,
...
controller.responderNotificacion = (req, res) => {
  ...
  req.getConnection((err, conn) => {
    if (err) return res.status(500).json({ error: "Error conexión" });
    conn.query(
      `SELECT * FROM pol_notificaciones WHERE id_Notificacion = ?`,
      [id_Notificacion],
      (err, rows) => {
        if (err || !rows.length) {
        ...
        const notif = rows[0];
        conn.query(
          `INSERT INTO pol_notificaciones_usuarios (id_notificacion, id_usuario, leida, fecha_lectura) VALUES (?, ?, 1, NOW()) ON DUPLICATE KEY UPDATE leida = 1`,
          [id_Notificacion, id_usuario],
        );
        if (respuesta === "aceptada") {
          conn.query(
            `SELECT Estado FROM pol_notificaciones WHERE id_Notificacion = ?`,
            [id_Notificacion],
            (err, result) => {
              if (err || !result.length) {
                return res
                  .status(500)
                  .json({ error: "Error verificando estado" });
              }
              ...
              if (estadoActual === "aceptada") {
                return res.json({
                  success: false,
                  message:
                    "Esta notificación ya fue procesada por otro usuario",
                });
              }
              const detalle = `Movil: ${notif.Movil}, Personal: ${notif.PersonalCustodio}, Obs: ${notif.Observaciones}`;
              conn.query(
                `INSERT INTO pol_internomovimiento (id_InternoLegajo, Unidad_Alojado, Unidad_Destino, Tipo_Movimiento, Detalle, Usuario) VALUES (?, ?, ?, 'TRASLADO', ?, ?)`,
                [
                  notif.id_InternoLegajo,
                  notif.Unidad_Origen_Codigo,
                  notif.Unidad_Destino_Codigo,
                ...
                (err) => {
                  ...
                  conn.query(
                    `UPDATE pol_notificaciones SET Estado = 'aceptada', Leida = 1 WHERE id_Notificacion = ?`,
                    [id_Notificacion],
                  );
                  return res.json({
                    success: true,
          ...
          conn.query(
            `UPDATE pol_notificaciones SET Estado = 'rechazada' WHERE id_Notificacion = ?`,
            [id_Notificacion],
          );
...
controller.buscarInterleg = async (req, res) => {
  try {
    ...
    // Consultar MySQL usando req.getConnection
    req.getConnection((err, conn) => {
      conn.query(
        `SELECT pe.Apellido, pe.Nombre, pe.Dni, fo.frente AS fotoUrl, le.id_InternoLegajo, le.estado AS estadoLegajo, (SELECT Unidad_Destino FROM pol_internomovimiento WHERE id_InternoMovimiento = (SELECT MAX(id_InternoMovimiento) FROM pol_internomovimiento WHERE id_InternoLegajo = le.id_InternoLegajo)) AS Unidad_Alojamiento, (SELECT pol_unidades.Detalle FROM pol_internomovimiento INNER JOIN pol_unidades ON pol_internomovimiento.Unidad_Destino = pol_unidades.id_unidades WHERE id_InternoLegajo = le.id_InternoLegajo ORDER BY id_InternoMovimiento DESC LIMIT 1) AS Detalle_Alojamiento FROM pol_persona pe INNER JOIN pol_internolegajo le USING(id_persona) LEFT JOIN pol_internofotos fo USING(id_InternoLegajo) WHERE pe.Apellido LIKE ? OR pe.Nombre LIKE ? OR pe.Dni LIKE ?`,
        [`%${q}%`, `%${q}%`, `%${q}%`],
        (err, results) => {
          if (err) {
...
controller.consultaDetenido = async (req, res) => {
  ...
  try {
    ...
    req.getConnection((err, conn) => {
      ...
      conn.query(
        `SELECT pe.Nombre, pe.Apellido, pe.Dni, le.id_InternoLegajo, fo.frente FROM pol_persona pe INNER JOIN pol_internolegajo le USING(id_persona) LEFT JOIN pol_internofotos fo USING(id_InternoLegajo) WHERE pe.Dni = ? LIMIT 1`,
        [dni],
        (err, results) => {
          if (err) {
            return res.status(500).json({ error: true });
